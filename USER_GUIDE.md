...
**Примечание:** Проект использует библиотеку **Whoosh** для полнотекстового поиска. Она устанавливается автоматически вместе с остальными зависимостями.
...
5. **Просмотр данных и поиск** — использование веб-интерфейса и полнотекстового поиска
...
```powershell
# Используйте скрипт запуска (автоматически проверит и установит зависимости)
.\start.ps1
```

Или дважды кликните на `start.bat` в проводнике Windows.

**Linux/Mac или вручную:**
```bash
...
**Примечание:** Скрипт `start.ps1` (или `start.bat`) автоматически:
- Проверит наличие Python
- Создаст виртуальное окружение, если его нет
- Установит все зависимости, если они не установлены
- Проверит/установит Playwright браузер
...
- Поддерживаемые продукты
- Категории
- **Кнопка "Documentation"** — ссылка на документацию вендора (если доступна)
- Список версий с:
  - Номером версии
  - Датой выпуска
  - Информацией о совместимости
  - Ссылкой на загрузку (если файл загружен)
  - **Release Notes** с правильным отображением HTML (раскрывающиеся блоки)

#### Поиск (`/search`)

**Полнотекстовый поиск по описаниям плагинов и Release Notes**

Отображает:
- Поисковую строку для ввода запроса
- Результаты поиска с:
  - Названием плагина
  - Вендором и ключом приложения
  - Бейджами продуктов
  - Типом совпадения (Description, Release Notes, или оба)
  - Контекстом совпадения (фрагмент текста с найденным запросом)
  - Ссылкой на страницу деталей плагина
...
**Примеры запросов:**
- `tables in jira` — найти плагины для работы с таблицами в Jira
- `confluence automation` — найти плагины для автоматизации в Confluence
- `workflow AND jira` — найти плагины для workflow в Jira
- `table*` — найти все плагины, содержащие слова, начинающиеся с "table"
...
**Доступные задачи:**
1. **Scrape Apps** — сбор приложений из Marketplace
2. **Scrape Versions** — сбор версий приложений
3. **Download Binaries** — загрузка бинарных файлов
4. **Download Descriptions** — скачивание описаний плагинов
5. **Build Search Index** — построение поискового индекса (новое)
6. **Full Pipeline** — запуск всех задач последовательно
...
- **Автоматическое включение кнопок** после завершения задач
- **Отслеживание прогресса индексации** в реальном времени

**Задача "Build Search Index":**
- Запускается вручную через кнопку "Build Search Index"
- Показывает прогресс индексации (количество обработанных плагинов)
- Индексирует все описания (JSON, HTML) и release notes
- Индекс используется для быстрого поиска на странице `/search`
- Рекомендуется запускать после скачивания новых описаний
...
**Пример:**
```bash
curl "http://localhost:5000/api/search?q=table%20grid"
```
...
```json
{
  ...
  "results": [
    {
      "addon_key": "com.example.app",
      "app_name": "Example App",
      "vendor": "Example Vendor",
      "match_type": "description",
      "match_context": "...found text...",
      "products": ["jira"],
      "score": 0.95
    }
  ],
...
Перестроить поисковый индекс (требует аутентификации).
...
### Q: Как работает поиск по описаниям плагинов?

**A:** Поиск использует библиотеку Whoosh для полнотекстового поиска:
- Индексируются все описания: JSON, full page HTML, release notes
- HTML теги автоматически удаляются перед индексацией
- Поддерживаются сложные запросы (фразы, wildcards, boolean операторы)
- Результаты ранжируются по релевантности
- Индекс автоматически перестраивается при первом использовании
- Индекс хранится в `DESCRIPTIONS_DIR/.whoosh_index/`

**Примеры запросов:**
- `table grid` — поиск фразы
- `table*` — поиск слов, начинающихся с "table"
- `table AND jira` — поиск плагинов, содержащих "table" и "jira"
- `automation OR workflow` — поиск плагинов, содержащих "automation" или "workflow"

### Q: Почему кнопка "Documentation" серая и неактивная?

**A:** Кнопка становится неактивной, если:
- Ссылка на документацию не была найдена на странице Marketplace
- Ссылка не была извлечена при скачивании описания
- Описание плагина еще не было скачано

**Решение:** Скачайте описание плагина заново, чтобы извлечь ссылку на документацию:
```bash
python run_description_downloader.py --addon-key com.example.app --download-media
```

### Q: Как перестроить поисковый индекс?

**A:** Индекс автоматически перестраивается при первом использовании, если его нет. Для ручного перестроения:
1. **Рекомендуемый способ:** Используйте веб-интерфейс: Management → Build Search Index. Это запустит задачу с отслеживанием прогресса.
2. **Через API:** Используйте endpoint `POST /api/tasks/start/build-index` (требует аутентификации)
3. **Через командную строку:** Запустите `python run_index_search.py`
4. **Автоматически:** Удалите папку `.whoosh_index` в директории описаний — индекс перестроится автоматически при следующем поиске

**Примечание:** После скачивания новых описаний плагинов рекомендуется перестроить индекс для включения их в поиск.
