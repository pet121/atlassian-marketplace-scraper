...
- Сохраняется страница в формате MHTML (один файл со всеми ресурсами)
...
- Формат MHTML включает все ресурсы (CSS, изображения) в одном файле
- Работает оффлайн после скачивания

**Результат:**
- MHTML файлы в `{DESCRIPTIONS_DIR}/{addon_key}/full_page/index.mhtml`
...
#### Страница управления (`/manage`)

Централизованная панель управления для запуска задач и мониторинга их выполнения.

**Разделы:**

1. **Task Control (Управление задачами)**
   - Кнопки для запуска различных задач:
     - "1. Scrape Apps" — сбор приложений
     - "2. Scrape Versions" — сбор версий
     - "3. Download Binaries" — загрузка бинарных файлов
     - "4. Download Descriptions" — скачивание описаний плагинов
   - Кнопка "Full Pipeline" — последовательный запуск всех задач

2. **Task Status (Статус задач)**
   ...
   **Таблица задач содержит:**
   - Task ID — идентификатор задачи
   - Script — имя скрипта
   - Status — статус (running, completed, failed, cancelled)
   - Current Action — текущее действие задачи
   - **Last Log Line** — последняя строка из лог-файла (обновляется каждые 5 секунд)
   ...
   - Actions — кнопки действий:
     ...
     - **View Error** — просмотреть ошибку для failed задач
   
   **Функции:**
   - **Refresh** — обновить статус задач вручную
   - **Clear Completed** — удалить все завершенные/неудачные/отмененные задачи из списка
   - **Last update** — время последнего обновления статуса
   
   **Мониторинг логов:**
   - Для выполняющихся задач автоматически отображается последняя строка из соответствующего лог-файла
   - Логи обновляются каждые 5 секунд
   - При наведении курсора показывается полный текст лог-строки
   - Маппинг скриптов на лог-файлы:
...
3. **Settings (Настройки)**
   - Редактирование параметров скрапера через веб-интерфейс
   - Сохранение изменений в `.env` файл

4. **Storage Paths (Пути хранения)**
   - Просмотр и изменение путей для хранения данных
   - Настройка путей для разных продуктов

5. **Marketplace API Credentials (Учетные данные)**
   - Управление username и API token
   - Хранение в `.credentials.json` (безопасно для git)

6. **View Logs (Просмотр логов)**
   - Выбор лог-файла из списка
   - Просмотр последних N строк (по умолчанию 500)
